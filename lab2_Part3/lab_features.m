...
%
    ...
    % RGB Histogram   
    % First of all, define the colors by RGB format[0-255,0-255,0-255,] 
    rgb_bucket = [0,0,0; 0,0,255; 0,255,255; 255,0,0; 255,255,0;255,0,255; 0,255,0; 255,255,255];
    image_pixel = [];
    rgb_pixel = [];
    distance=[];
    
    % f4: fHistogramRGB, that need to be returned
    fHistogramRGB = zeros(1, size(rgb_bucket,1));   
    for i = 1: size(imgDownSampled, 1)
        for j = 1: size(imgDownSampled, 2)
            image_pixel = imgDownSampled(i, j,:);
            % get rgb pixel
            rgb_pixel = [image_pixel(:,:,1),image_pixel(:,:,2),image_pixel(:,:,3)];
            % get the distance between rgb pixel and bucket
            for b=1:size(rgb_bucket,1)
                dist = norm((rgb_pixel - rgb_bucket(b,:)),2);
                distance = [distance,dist]; 
            end
            % find the minimum distance to find which buck to add one
            [~,buck_index] = sort(distance);
            fHistogramRGB(:,buck_index(1)) = fHistogramRGB(:,buck_index(1))+1;
            distance = [];
        end
    end
    ...
    % f5: imgAreaRGB, that need to be returned
    imgAreaRGB = lab_downsample(img,RGB_AREA_GRID_W,RGB_AREA_GRID_H);
    fAreaRGB = [];
    % Divides the image up into a grid
    for y=1:size(imgAreaRGB, 1)
        for x=1:size(imgAreaRGB, 2)
            % calculate the average color of each grid cell.
    ...
    f2 = fTexture;
    f3 = fHistogramGrey;
    % return fHistogramRGB and fHistogramRGB
    f4 = fHistogramRGB;
    f5 = fAreaRGB;
